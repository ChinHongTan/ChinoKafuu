    ...
    execute(message, imageChannels) {
        ...
        async function fetchImageChannels(imageChannels) {
            imageChannels.forEach((ID) => {
                totalImageCount.push(
                    new Promise(function (resolve, reject) {
                        var image = [];
                        var numbers = new Object();
                        message.client.channels.fetch(ID).then((channel) => {
                            console.log(`Fetching ${channel.name}`);
                            var total = 0;
                            (async () => {
                                for await (const messages of loadAllMessages(
                                    channel
                                )) {
            ...
            await Promise.all(totalImageCount).then((value) => {
                const loliPic25 = guild.roles.cache.find(
                    (role) => role.id === "765215141641060412"
                );
                const explicitPic25 = guild.roles.cache.find(
                    (role) => role.id === "765571817003286569"
                );
                const vtuberPic25 = guild.roles.cache.find(
                    (role) => role.id === "766832086836707348"
                );
                const tolPic300 = guild.roles.cache.find(
                    (role) => role.id === "797711580967862272"
                );
                const tolPic500 = guild.roles.cache.find(
                    (role) => role.id === "797711805308207165"
                );
                const tolPic700 = guild.roles.cache.find(
                    (role) => role.id === "800730178090303488"
                );
                const tolPic1000 = guild.roles.cache.find(
                    (role) => role.id === "781523067397865482"
                );
                ...
                for (const [name, number] of Object.entries(imageCount)) {
                    ...
                    value.forEach((channelInfo) => {
                        ...
                        channelID = guild.channels.cache.find(
                        ...
                        if (name in picsInChannel) {
                            ...
                            if (
                                user &&
                                picsInChannel[name] >= 25 &&
                                channelID === "764840462707326986"
                            ) {
                                user.roles.add(loliPic25);
                                console.log(
                                    `Added ${loliPic25.name} to ${user.user.username}`
                                );
                            }
                        }
                    });
                    ...
                    var vtuber =
                        imagesSendByUser[
                            guild.channels.cache.find(
                                (channel) => channel.id === "768430617733496842"
                            ).name
                        ] ?? 0; //vtuber
                    var vtuber18r =
                        imagesSendByUser[
                            guild.channels.cache.find(
                                (channel) => channel.id === "766636820066074656"
                            ).name
                        ] ?? 0; //vtuber 18r
                    var gura =
                        imagesSendByUser[
                            guild.channels.cache.find(
                                (channel) => channel.id === "765525905807769621"
                            ).name
                        ] ?? 0; //gura
                    var _18r =
                        imagesSendByUser[
                            guild.channels.cache.find(
                                (channel) => channel.id === "765176655407874081"
                            ).name
                        ] ?? 0; //18r
                    ...
                    if (user && vtuberSum >= 25) {
                        ...
                        console.log(
                            `Added ${vtuberPic25.name} to ${user.user.username}`
                        );
                    }
                    if (user && _18rSum >= 25) {
                        user.roles.add(explicitPic25);
                        console.log(
                            `Added ${explicitPic25.name} to ${user.user.username}`
                        );
                    }
                    ...
                    if (user && number >= 300) {
                        ...
                        console.log(
                            `Added ${tolPic300.name} to ${user.user.username}`
                        );
                    }
                    if (user && number >= 500) {
                        user.roles.add(tolPic500);
                        console.log(
                            `Added ${tolPic500.name} to ${user.user.username}`
                        );
                    }
                    if (user && number >= 700) {
                        user.roles.add(tolPic700);
                        console.log(
                            `Added ${tolPic700.name} to ${user.user.username}`
                        );
                    }
                    if (user && number >= 1000) {
                        user.roles.add(tolPic1000);
                        console.log(
                            `Added ${tolPic1000.name} to ${user.user.username}`
                        );
                    }
                ...
                profileWithTimestamp[new Date().toLocaleString()] =
                    memberProfile;
