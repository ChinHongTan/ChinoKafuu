...
module.exports = {
    ...
    "balance":"查看你的余额",
    "userBalance":"${target.tag} 有 ${currency.getBalance(target.id)}💰",
    "buy":"从商店中买东西",
    "noItem":"❌ | 这东西不存在",
    "notEnoughMoney":"❌ | 你目前有 ${currency.getBalance(message.author.id)}, 但是 ${item.name} 要价 ${item.cost}!",
    "noMoney":"❌ | 你不够钱买 ${item.name}，它要价 ${item.cost}!",
    "buySucess":"✅ | 你购买了: ${item.name}.",
    "inventory":"查看你的物品",
    "noInventory":"❌ | ${target.tag} 什么也没有!",
    "showInventory":"${target.tag} 目前有 ${items.map((i) => `${i.amount} ${i.item.name}`).join(', ')}",
    "leaderboard":"查看金钱排行榜",
    "shop":"查看商店物品",
    "transfer":"给其他人送钱",
    "invalidAmount":"❌ | 很抱歉 ${message.author}, 这个数额不正确",
    "noTransferTarget":"❌ | 请说清楚你要给谁钱!",
    "noMoenyTransfer":"❌ | 很抱歉 ${message.author}, 你只有 ${currentAmount}.",
    "negativeAmount":"❌ | 请输入大于零的数字, ${message.author}.",
    "transferSucess":"✅ | 成功把 ${transferAmount}💰 给了 ${transferTarget.tag}. 你现在有 ${currency.getBalance(message.author.id)}💰",
    "8ball":"问问神奇八号球~",
    "reply1":"这是必然",
    "reply2":"肯定是的",
    "reply3":"不用怀疑",
    ...
    "loli":"萝莉图",
    "loliPic":"萝莉!",
    "loliProvider":"来源商：${images.author}\n[图片链接](${messageurl})",
    "youtube":"一起看YouTube!",
    "notInVC":"❌ | 加入语音频道后才能使用此指令！",
    "anime":"根据动漫截图查找动漫/查询动漫相关信息",
